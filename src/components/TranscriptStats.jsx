...
function TranscriptStats({ instStats, transferEarned, setTransferEarned, institution }) {
  const overallEarned = instStats.earned + (Number.isFinite(transferEarned) ? transferEarned : 0);
    ...
    <section id="transcriptStats" className="mt-10">
      ...
      <div className="w-full bg-white rounded-2xl ring-1 ring-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-12 border-b border-gray-100 items-center text-sm">
          ...
          <div className="col-span-2 px-4 py-2 text-center">
            {fmt(instStats.gpa, institution)}
          </div>
          <div className="col-span-2 px-4 py-2 text-center">
            {fmt(instStats.attempted, institution)}
          </div>
          <div className="col-span-2 px-4 py-2 text-center">
            {fmt(instStats.earned, institution)}
          </div>
          <div className="col-span-3 px-4 py-2 text-center">
            {fmt(instStats.qp, institution)}
          </div>
        </div>
        {/* Overall Totals Row */}
